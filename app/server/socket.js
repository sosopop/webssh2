...
module.exports = function socket(socket) {
  ...
  if (
    ...
    if (!matcher.contains(socket.request.session.ssh.host)) {
      console.log(
        "WebSSH2 " +
          "error: Requested host outside configured subnets / REJECTED".red
            .bold +
          " user=" +
  ...
  conn.on("ready", function connOnReady() {
    console.log(
      "WebSSH2 Login: user=" +
        ...
        socket.id +
        " mrhsession=" +
        socket.request.session.ssh.mrhsession +
        " allowreplay=" +
        socket.request.session.ssh.allowreplay +
        " term=" +
        socket.request.session.ssh.term
    );
    if (socket.request.session.proxy) {
      console.log("proxy: " + socket.request.session.proxy);
      console.log("proxy-port: " + socket.request.session.proxyPort);
      console.log("proxy-user: " + socket.request.session.proxyUser);
      console.log("proxy-pass: " + socket.request.session.proxyPass);
    }
  ...
  conn.on(
    ...
    function connOnKeyboardInteractive(
      name,
      instructions,
      instructionsLang,
      prompts,
  ...
  if (
    ...
    (socket.request.session.userpassword ||
      socket.request.session.privatekey) &&
    ...
    if (socket.request.session.proxy) {
      SocksClient.createConnection({
        proxy: {
          host: socket.request.session.proxy,
          port: parseInt(socket.request.session.proxyPort, 10),
          type: 5,
          userId: socket.request.session.proxyUser,
          password: socket.request.session.proxyPass,
        },
        command: "connect",
        destination: {
          host: socket.request.session.ssh.host,
          port: parseInt(socket.request.session.ssh.port, 10),
        },
      })
        .then((info) => {
          console.log(info);
  ...
  function SSHerror(myFunc, err) {
    ...
    if (socket.request.session) {
      ...
      if (err && err.level === "client-authentication") {
        console.log(
          "WebSSH2 " +
            "error: Authentication failure".red.bold +
            " user=" +
            socket.request.session.username.yellow.bold.underline +
            " from=" +
            socket.handshake.address.yellow.bold.underline
